module Pact
  class ConsumerContract
    module ResponseFinder

      def get_response_contract(provider_name, consumer_name, provider_state = nil)
      ...
      def get_response_sample(provider_name, consumer_name, provider_state = nil)
        Pact::Reification.from_term(get_response_contract(provider_name, consumer_name, provider_state))
      end
      ...
      def pact_broker_url(provider, consumer)
        ...
        URI(ENV.fetch('PACT_BROKER_URL', 'http://pact-broker') + url + 'latest')
      end
